...
class ScaleFreeNetwork:
    ...
    def add_node(self):
        ...
        # Update leader adjacencies
        leaders = self.elect_leaders()
        for leader in leaders:
            if leader not in self.leader_adjacencies:
                self.leader_adjacencies[leader] = set()
            self.leader_adjacencies[leader].add(new_node_id)
    ...
    def get_leader_adjacencies(self, leader):
        if leader in self.leader_adjacencies:
            return len(self.leader_adjacencies[leader])
        else:
            return 0
...
class Blockchain(SimpleBlockchain):
    def __init__(self, difficulty=2):
        ...
        self.leader_adjacencies = {}
        self.node_connection_frequency = {}
    ...
    def calculate_leader_adjacencies(self):
        leaders = self.network.elect_leaders()
        for leader in leaders:
            adjacencies = self.network.get_leader_adjacencies(leader)
            self.leader_adjacencies[leader] = adjacencies
            for node in self.network.leader_adjacencies[leader]:
                if node in self.node_connection_frequency:
                    self.node_connection_frequency[node] += 1
                else:
                    self.node_connection_frequency[node] = 1

    def get_leader_adjacencies(self, leader):
        if leader in self.leader_adjacencies:
            return self.leader_adjacencies[leader]
        else:
            return 0
        
    def check_leader_adjacency(self):
        self.calculate_leader_adjacencies()
        total_adjacency = sum(self.leader_adjacencies.values())
        total_nodes = len(self.network.nodes)
        required_adjacency = 0.51 * total_nodes

        if total_adjacency >= required_adjacency:
            print(f"The 10 leader nodes have at least 51% adjacency in the network of 100 nodes.")
        else:
            print(f"The 10 leader nodes do not have at least 51% adjacency in the network of 100 nodes.")

        # Print the connected nodes for each leader
        leaders = self.network.elect_leaders()
        for leader in leaders:
            connected_nodes = self.network.leader_adjacencies[leader]
            connected_nodesNumber = self.leader_adjacencies[leader]
            print(f"Leader node {leader} is connected to {connected_nodesNumber} following nodes.")
            for node in connected_nodes:
                print(f"Node {node}")

        # Print the frequency of node connections
        print("\nFrequency of node connections:")
        for node, frequency in self.node_connection_frequency.items():
            print(f"Node {node} is connected to {frequency} leader nodes.")
...
blockchain = Blockchain(difficulty=2)
blockchain.network = ScaleFreeNetwork(initial_nodes=3, m=7)
while len(blockchain.network.nodes) < 100:
    blockchain.network.add_node()

blockchain.check_leader_adjacency()



network = ScaleFreeNetwork(initial_nodes=3, m=7)
while len(network.nodes) < 100:
    network.add_node()

leaders = network.elect_leaders()
for leader in leaders:
    adjacencies = network.get_leader_adjacencies(leader)
    print(f"Leader node {leader} is connected to {adjacencies} other nodes.")
